...
/**********
  * @brif       该驱动文件用于驱动DHT11温湿度模块
  *
  * @attention  # 使用该驱动模块不需要提前初始化,
				# 两次读取数据的间隔应该大于2两秒
				# 尽量使用5V供电，3.3V供点电可能会导致读出的数据全为0
  *
  ==============================================================================
                        ##### 如何使用该驱动文件 #####
  ==============================================================================
  (1)、对GPIO引脚进行配置
	方法一: 使用CubeMX，任意选择一个未被使用的引脚，将该引脚设置为输出模式，并将该引脚重命名为: DHT11
	方法二：不使用CubeMX配置，直接在 dht11.h 中对引脚进行宏定义，具体配置步骤请在 dht11.h 中查看
  
  (2)、使用 DHT11_READ_DATA(float *temp, float *humi) 进行温湿度数据的读取。
	   需要提前定义两个 float 型的变量，将变量地址作为函数的形参。下面是使用示例:
	   
	   float Temperature, Humidity;  // 定义变量，建议该条语句放在 Main() 函数之前，作为全局变量使用
	   
	   DHT11_READ_DATA(&Temperature,&Humidity);  // 注意：两次使用该函数的时间间隔应大于两秒
